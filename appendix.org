...
#
In particular, for each variable a separate regression model was run which regresses the variable in question at lag 0 against year while allowing slopes and intercepts to vary by country (year was set 0 at the beginning of the observation period, *X*).
#
*can only cover linear country trends*
#
...
#
For example the overall slope of GDP is *X* (indicating an average growth of GDP by *X*, or *X* USD), however the histogram shows that countries can substantially diverge from this overall slope as country-slopes can range from a minimum of *X* to a maximum of *Y*, with 75% of slopes lying between *X* and *K*.
#
