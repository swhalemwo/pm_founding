...
cor_tester <- function() {
    mtcars2 <- adt(mtcars) %>% copy()
    mtcars2[sample(.N, 16), `:=`(disp_overestmtd = disp*1.5,
                                 disp_undrestmtd = disp*0.5)] %>%
        .[is.na(disp_overestmtd), `:=`(disp_overestmtd = disp,
                                       disp_undrestmtd = disp)]
    cor_overestmtd <- cor(mtcars2$disp_overestmtd, mtcars2$wt)
    cor_undrestmtd <- cor(mtcars2$disp_undrestmtd, mtcars2$wt)

    list(cor_overestmtd = cor_overestmtd,
         cor_undrestmtd = cor_undrestmtd)
}

x <- lapply(1:500, \(x) cor_tester()) %>% rbindlist()
...
## actual data:
df_excl2 <- create_excel_df(PMDB_FILE, only_pms = F) %>% adt()
df_excl2[, .N, Museum_status]

df_excl2[Museum_status != "private museum", .N, is.na(`Museum_closing year`)]

df_excl2[Museum_status != "private museum", .N, Museum_country][order(-N)]

df_excl2[Museum_status != "private museum", .N, by = .(year = as.numeric(`Museum_opening year`))] %>% 
    ggplot(aes(x=year, y=N)) + geom_col()

df_excl2[Museum_status != "private museum" & is.na(`Museum_closing year`), .N,
         by = .(year = as.numeric(`Museum_opening year`))] %>% 
    ggplot(aes(x=year, y=N)) + geom_col()


dt_pred_closing <- df_excl2[!is.na(`Museum_closing year`)  & !is.na(`Museum_opening year`)] %>% copy() %>% 
    .[, `:=`(closing_year = as.integer(`Museum_closing year`),
             opening_year = as.integer(`Museum_opening year`))] %>%
    .[, nbr_years_open := closing_year - opening_year]

    ## .[, mean(nbr_years_open)]

lm_pred1 <- lm(closing_year ~ opening_year, dt_pred_closing)
lm_pred2 <- lm(nbr_years_open ~ opening_year, dt_pred_closing)

summary(lm_pred2)


ggplot(dt_pred_closing, aes(x=opening_year, y=closing_year)) + geom_point()


dt_topred <- df_excl2[is.na(`Museum_closing year`) & !is.na(`Museum_opening year`) &
                      Museum_status != "private museum"] %>%
    .[, opening_year := as.integer(`Museum_opening year`)]
    


dt_topred[, `:=`(closing_year1 = predict(lm_pred, dt_topred),
                 nbr_years_open = predict(lm_pred2, dt_topred))] %>%
    .[, closing_year2 := opening_year + nbr_years_open]

dt_topred[, .(closing_year1, closing_year2)] %>%
    .[, .(diff = closing_year1 - closing_year2)] %>% .[, diff]


                 ][, closing_year]

nbr_years_open := predict(lm_pred, dt_topred)] %>%
    .[, closing_year := opening_year + nbr_years_open] %>%
    .[, closing_year]
