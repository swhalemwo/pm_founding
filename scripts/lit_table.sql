...
SELECT pair FROM (
  SELECT DISTINCT(CONCAT(country_name, '-', counterpart_country_name)) AS pair FROM dots_prep WHERE time_period='1985') LEFT JOIN (
    SELECT DISTINCT(CONCAT(country_name, '-', counterpart_country_name)) AS pair FROM dots_prep WHERE time_period='2020') USING pair
