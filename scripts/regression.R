...
gen_vrbl_thld_choices <- function() {
...
vrbl_thld_choices <- gen_vrbl_thld_choices()


gen_reg_spec <- function() {
    #' generate the regression specification

    ## could also just sample here, probably have to
    x <- vrbl_choices[sample(nrow(vrbl_thld_choices),1),]
...
gen_lag_df(lngtd_vars, crscn_vars, base_vars)
