...
run_glmmtmb_wctrl <- function(dfx, dvfmt, r_vars, verbose, glmmtmb_control) {
    ...
    rx_glmmtmb <- glmmTMB(fx, dfx, family = nbinom1, verbose = verbose, control = glmmtmb_control)
...
glmmtmb_control1 <- glmmTMBControl(optCtrl = list(iter.max = 20), profile = T)




t1 <- Sys.time()
run_vrbl_mdl_vars(x, vvs, fldr_info_optmz, verbose = F, wtf = F, glmmtmb_control = glmmtmb_control1)
t2 <- Sys.time()
t2-t1
