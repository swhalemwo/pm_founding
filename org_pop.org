...
#
For example, while in "DS all IVs" each year on average 1 private museum is founded per 90.9 million inhabitants (1/0.011), this rate is substantially lower in "DS --CuSP" and "DS -CuSp/TMITR" with 1 private museum founding per 166.6 and 200 million inhabitants, respectively.
#
...
Table \ref{tbl:regrslts_wcptblF} displays the regression results (as the estimation procedure involves the calculation of multiple models from different variable sets, table X displays for each variable the coefficients of the best-fitting models; the log likelihoods similarly correspond to the best-fitting model. As the coefficient variation resulting from different variable choice (e.g. the difference between the coefficients of cultural spending between a model with income inequality measured via the Gini coefficient and a model with income measured as 10% concentration) is generally not substantial, it is discussed only in the Appendix). 
# 
Coefficients can be interpreted as logged multipliers of a country's founding rate [[parencite:&Coxe_West_Aiken_2009_count]], therefore a coefficient of 0.2 corresponds to a exp(0.25) = 1.28 multiplier, i.e. a 28% increase in average country founding rate given a 1 standard deviation change in the independent variable (as the independent variables are standardized).
#
...
#
Thus they also illustrate the highly skewed distribution of the variables, as predicted opening count at the means for e.g. "DS all IVs" with exp(-2.22)=0.10 is around one order of magnitude higher than average founding rate within the dataset (0.011, table \ref{tbl:descs}); a pattern of similar magnitude is present in the other datasets as well. 
