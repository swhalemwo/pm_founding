...
#
Given that the smaller datasets, especially "DS all IVs", countries of the Global North are overrepresented, these datasets are characterized disproportionately by higher rates of HNWIs, lower inequalities, higher GDP per capita and larger (per capita) populations of museums and collectors of modern and contemporary art.
#
Furthermore the main variable of interest, private museum founding (and consequently private museum densities), is more prevalent in "DS all IVs" as well. 
#
For example, while in "DS all IVs" each year on average 1 private museum is founded per {{{nbr_pop_p_1pm_cbn_all}}} million inhabitants (1/{{{opng_rate_cbn1}}}), this rate is substantially lower in "DS --CuSP" and "DS -CuSp/TMITR" with 1 private museum founding per {{{nbr_pop_p_1pm_cbn_no_cult_spending}}} and {{{nbr_pop_p_1pm_cbn_no_cult_spending_and_mitr}}} million inhabitants, respectively.[fn::These rates refer to country-year averages which weigh all countries evenly regardless of population size. The dataset-year private museum founding rates are lower at on average one private museum founding per {{{opng_p1m_glbl_yearly_cbn_all}}}, {{{opng_p1m_glbl_yearly_cbn_no_cult_spending}}}, {{{opng_p1m_glbl_yearly_cbn_no_cult_spending_and_mitr}}} million inhabitants in "DS all IVs", "DS --CuSP", and "DS -CuSp/TMITR", respectively, indicating that private museum founding takes place disproportionally in country-years with population size below average]
#
Additionally, several variables show substantial skew (indicated by high standard deviations compared to the mean), such as cultural spending, GDP and the HNWI, art museum and collector rates, as well as the dependent variable of private museum openings: 
#
...
#
A 1 SD increase of the top marginal income tax rate ({{{tmitr_1SD_cbn_no_cult_spending}}} percentage points, corresponding approximately to the difference between the Netherlands ({{{tmitr_iso3c1}}}%) and Chile ({{{tmitr_iso3c2}}}%) in 2020) is associated with a difference in founding rate that is e\textsuperscript{{{{txdctblt_tmitr_interact_cbn2}}}} = {{{txdctblt_tmitr_interact_cbn2_exp}}} times larger in countries with tax deductibility of donations than in countries without.
# 
Given that the effect of an additional 1 SD top marginal income tax rate for countries without tax deductibility of donations is {{{tmitr_cbn2}}}, a 1 SD increase in top marginal income tax rate in countries with tax deductibility of donations corresponds to an increase in log-founding by {{{tmitr_cbn2}}} + {{{txdctblt_tmitr_interact_cbn2}}} = {{{tmitr_neteffct_txdctblt1_cbn_no_cult_spending}}}, or an increase in founding rate by a factor of e\textsuperscript{{{{tmitr_neteffct_txdctblt1_cbn_no_cult_spending}}}} = {{{tmitr_neteffct_txdctblt1_cbn_no_cult_spending_exp}}}. 
# 
# increases the effect of tax deductibility of donations by a factor of .
#
In "DS all IVs", the interaction is still positive, but no longer significant, possibly because "DS all IVs" is primarily composed of countries in which charitable donations are tax deductible (table \ref{tbl:descs}) and hence little variation exists.
#
Thus despite significant effects not being present in every dataset, the data overall indicates substantial support for an association between tax incentives and private museum founding, supporting H1. 
...
#
Consequently, there is little evidence that HNWI rates are substantially associated with private museum founding in either direction (H4). 
...
#
Indicated by the negative coefficient of the squared density measure (which significant everywhere except for the global coefficient in "DS all IVs"; where however it is marginally significant), this is the case for density measures on the level of both the country and the world.
# 
Thus following the density dependence paradigm, potential founders were motivated by their peers' decision to establish a private musuem up to the point where private museum numbers increased to the extent that additional founding did not appear advantageous to them anymore. 
#
...
***** Art field indicators
#
The coefficients of the number of museums of modern and contemporary art and of the number of collectors in the Artnews top collector ranking are relatively small and insignificant.
#
