...
#+MACRO: shweal_cbn_all 0.28
#+MACRO: shweal_cbn_all_exp 1.32
...
#+MACRO: sptinc_cbn_all -0.39
#+MACRO: sptinc_cbn_all_exp 0.68
...
#
Currently, the database includes {{{nbr_muem_in_pmdb}}} museums (of which *446* are currently open), located in {{{nbr_cry_wal1pm_all}}} countries, with the majority being located in Western Europe (*136*), East Asia (*97*) and North America (*77*).
#
...
# 
Conversely, a 1 SD increase of the wealth share of the top 10% ({{{shweal_1SD_cbn_all}}}% in 2020 in "DS all IVs", a difference in size similar to that between Denmark ({{{shweal_iso3c1}}}%) and the Czech Republic ({{{shweal_iso3c2}}}%) in 2020, or to the longitudinal change in the United States from {{{shweal_lngtd_year1}}} ({{{shweal_lngtd_vlu_year1}}}%) to {{{shweal_lngtd_year2}}} ({{{shweal_lngtd_vlu_year2}}}%)) is associated with an increase in private museum founding rate by a factor of e\textsuperscript{{{{shweal_cbn_all}}}} = {{{shweal_cbn_all_exp}}}.
#
...
#
Either or both of these patterns possibly lead to the gini coefficients in "DS all IVs" to become not significant.
# 
Investigating the mechanisms behind patterns constitutes fruitful approaches for future research. 



**** HNWI :ignhead:

Increases in the number of HNWI do not seem to be substantially associated with private museum founding as after controlling for other variables, the HNWI coefficients are both comparatively small relative to to those of other indicators and generally not statistically significant.
# 
The point estimate are furthermore often negative, as are the two HNWI coefficients who do reach statistical significance (HNWI >= 5M in "DS -CuSp" and "DS -CuSp/TMITR").
#
While thus some evidence might even point to a slight negative association between HNWI numbers and private museum founding, the small and often insignificant coefficients as well as the inconsistencies of coefficient signs does for now not provide substantial evidence in either direction. 
...
In all datasets,  measures of density (legitimacy) and density squared (competition) correspond to the density-dependence hypotheses, namely a reverse U-shaped relationship between density and private museum founding.
#
