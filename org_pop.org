...
#
For example, a 1 SD increase of the wealth share of the top 10% ({{{shweal_1SD_cbn_all}}}% in 2020 in "DS all IVs", which corresponds approximately to the difference between Denmark ({{{shweal_iso3c1}}}%) and the Czech Republic ({{{shweal_iso3c2}}}%) in 2020, or to the longitudinal change in the United States from {{{shweal_lngtd_year1}}} ({{{shweal_lngtd_vlu_year1}}}%) to {{{shweal_lngtd_year2}}} ({{{shweal_lngtd_vlu_year2}}}%)) is associated with an increase in private museum founding rate by a factor of e\textsuperscript{{{{shweal_cbn_all}}}} = {{{shweal_cbn_all_exp}}}.
#

and in all datasets larger in the top percentile shares than the 
#
Furthermore, 
...
# 
First, the effects of wealth inequality are substantially larger in "DS -CuSP" and "DS -CuSp/TMITR".
#
Second in all datasets the coefficients of the top percentile shares are larger (in absolute terms) than those of the gini coefficients.
#
Either or both of these patterns possibly lead to the gini coefficients in "DS all IVs" to become not significant.
#
Investigating the mechanisms behind patterns is however beyond the scope of the current research.
