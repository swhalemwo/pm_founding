...
**** Inequality
The WID provides Gini coefficients for wealth and pre-tax income[fn::While data on post-tax income is also available in the WID, its coverage is very limited].
#
...
#
*FIXME: Additionally, growth of GDP per capita controlled for to capture changes in the*
#
...
#
Most country-years are situated between 2000 and 2020 with coverage generally increasing over time (figure {{{rplt_cbn_cycnt}}}); the datasets therefore effectively cover the period in which the majority of private museum founding has taken place (the datasets cover {{{perc_opngs_cvrd_cbn1}}}, {{{perc_opngs_cvrd_cbn2}}} and {{{perc_opngs_cvrd_cbn3}}} of foundings, respectively).
#
...
#
While the random starting values and random order of variables help to explore the lag space, models can still be stuck in local optima; I therefore run each model four times, each with a different set of random starting values (see figure {{{rplt_lag_dens}}}). 
...
#
Only {{{nbr_nzero_cbn1}}} country-years see show at least one private museum founding in "DS all IVs" ({{{prop_nzero_cbn1}}}%), while the other datasets see even lower proportions ({{{prop_nzero_cbn2}}}% for "DS --CuSP" and {{{prop_nzero_cbn3}}}% for "DS --CuSP/TMTIR", respectively).
...
#
While the coefficients are insignificant in the larger dataset, in DS all IVs it describes a U-shaped relationship, corresponding to higher rates of founding at the extreme ends of the scale.
#
