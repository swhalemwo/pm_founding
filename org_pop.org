...
# intcpt_info
#+MACRO: intcpt -4.64
#+MACRO: intcpt_exp 0.010


# txinctvs
#+MACRO: Ind_tax_incentives_cbn3_coef 0.68
#+MACRO: Ind_tax_incentives_cbn3_coef_exp 2.0
#+MACRO: Ind_tax_incentives_cbn3_pvlu 0.14
#+MACRO: Ind_tax_incentives_cbn2_coef 0.61
#+MACRO: tmitr_approx_linear20step_cbn2_coef -0.58
#+MACRO: ti_tmitr_interact_cbn2_coef 0.94
#+MACRO: ti_tmitr_interact_cbn2_coef_exp 2.57
#+MACRO: tmitr_1SD_cbn2 13.5
#+MACRO: tmitr_neteffct_txdctblt1_cbn2 0.37
#+MACRO: tmitr_neteffct_txdctblt1_cbn2_exp 1.44
#+MACRO: tmitr_iso3c1 50
#+MACRO: tmitr_iso3c2 36


# smorc_stats
#+MACRO: smorc_lin 0.59
#+MACRO: smorc_lin_flipped -0.59
#+MACRO: smorc_sqrd -0.65
#+MACRO: smorc_top_point_std 0.45
#+MACRO: smorc_top_point 485
...
#+MACRO: smorctop_below_CYs_cnt 864
#+MACRO: smorctop_above_CYs_cnt 377
#+MACRO: smorctop_below_nbr_opened_cnt 222
#+MACRO: smorctop_above_nbr_opened_cnt 133
#+MACRO: smorctop_below_CYs_prop 69.6
#+MACRO: smorctop_above_CYs_prop 30.4
#+MACRO: smorctop_below_nbr_opened_prop 62.5
#+MACRO: smorctop_above_nbr_opened_prop 37.5
...
#+MACRO: shweal_cbn1 0.34
#+MACRO: shweal_cbn1_exp 1.41
...
#+MACRO: sptinc_cbn1 -0.52
#+MACRO: sptinc_cbn1_exp 0.59
...
#
Combining various socio-economic and art field data sources, I conduct quantitative tests of these hypotheses with datasets of {{{nbr_cy_cbn1}}}, {{{nbr_cy_cbn2}}} and {{{nbr_cy_cbn3}}} country-years using multilevel negative binomial regression models.
#
...
# [fn::While themes of disinterestedness and intrinsic passion are strongly present in the self-presentations of founders [[parencite:&Bechtler_Imhof_2018_future;&Duron_2020_rebaudengo]], they might constitute strategic framings to adhere to art market norms [[parencite:&Velthuis_2007_talking]] rather than real motivations guiding actual behavior.]. 
#
...
# 
textcite:&Walker_2019_collector investigates the legal frameworks and tax exemptions that private museums receive in a number of locations such as Australia (p.31), the United States (p.35), Germany (p.29) and Switzerland (p.36), where she generally finds tax incentives to contribute to private museum founding. 
#
...
Proposing an impact of tax incentives does not imply a profit motive nor the absence of other motivations such as "passion for art" ([[cite:&Zorloni_Resch_2016_opportunities p.7]], [[cite:&Adam_2021_rise p.12]]) or other "personal motivations" [[parencite:&Walker_2019_collector p.144]]. 
#
In fact, other motivations (such as the aforementioned passion for the arts, as well as potential reputation gains; cf. [[textcite:&Bekkers_Wiepking_2010_literature;&Bekkers_Wiepking_2011_philanthropic]] for motivations of philanthropic behavior generally) are required as tax incentives only /reduce/ the cost of philanthropic behavior but do not cancel it out completely, let alone produce a financial profit; tax incentives thus may play a supportive role by easing the financial burdens of running a private museum as a consequence of other motivations.
...
#+label: tbl:data_srcs
#+caption: Concepts, operationalizations and data sources
...
| Density: Competition           | Number of PMs open^2 (country, neighbors, global)                           | PM Database                                              |
...
#
Currently, the database includes {{{nbr_muem_in_pmdb}}} museums (of which {{{nbr_pm_openatm}}} are currently open, the remainder having closed or transformed to the extent that they no longer constitute a private museum), which are located in {{{nbr_cry_wal1pm_all}}} countries, with the majority being located in Western Europe ({{{regsub_cnt_Western_Europe}}}), East Asia ({{{regsub_cnt_Eastern_Asia}}}) and North America ({{{regsub_cnt_Northern_America}}}).
#
The majority of them ({{{nbr_opnd_snc2k}}}, or {{{perc_opnd_snc2k}}}) have been founded after 2000.
# 
...
# This study has a global scope and includes a large number of country-years without private museum foundings to avoid sampling on the dependent variable. 
# 
# However both the time period and number of countries that can be investigated are limited by the coverage of the independent variables (see Analytical strategy for supplementary materials). 
...
I use data on government spending on culture, recreation and religion collected by the [[textcite:&UN_2022_consumption]], the [[textcite:&IMF_2022_GFS]], the [[textcite:&OECD_2022_SNA_TABLE11_ARCHIVE;&OECD_2022_SNA_TABLE11]] and [[textcite:&Eurostat_2022_COFOG]] as an indicator of cultural spending.
#
...
**** Inequality
The [[textcite:&WID_2021_WID]] provides Gini coefficients for wealth and pre-tax income[fn::While data on post-tax income is also available in the WID, its coverage is very limited].
#
...
# Private museums populations might interact with other museum populations in a manner similar to the density paradigm of legitimation of competition.
...
# doesn't require non-linear association
# An existing museum population might also facilitate private museum founding by providing a pool of skilled personnel for recruitment [[parencite:&Quemin_2020_power]].
#
...
Social fragmentation has been argued to decrease civic engagement parencite:&Costa_Kahn_2003_heterogeneity and public goods provision parencite:&Alesina_Baqir_1999_publicgoods, as it hinders the development of a collective identity [[parencite:&Habyarimana_etal_2009_coethnicity;&Baldassarri_Abascal_2020_diversity]].
#
...
#
I use the Historical Index of Ethnic Fractionalization [[parencite:&Drazanova_2020_HIEF]] and the international migrant stock (the percentage of people living in a country who are not born in it cite:&WB_2024_mist) to control for fragmentation along lines of ethnicity and citizenship, respectively. 
...
# 
In particular, I construct one dataset with all variables ({{{nbr_cy_cbn1}}} country-years, {{{nbr_crys_cbn1}}} countries; referred to as "DS all IVs"), one with all variables except cultural spending ({{{nbr_cy_cbn2}}} country-years, {{{nbr_crys_cbn2}}} countries; "DS --CuSp"), and one with all variables except cultural spending and top marginal income tax rates ({{{nbr_cy_cbn3}}} country-years, {{{nbr_crys_cbn3}}} countries; "DS --CuSp/TMITR").
#
In terms of composition, DS all IVs overrepresents Europe and underepresents the Global South, while the other datasets are more balanced (table {{{rtbl_cbn_cpsgn}}}).
#
...
# 
The datasets effectively cover the period in which the majority of private museum founding has taken place and cover {{{perc_opngs_cvrd_cbn1}}}, {{{perc_opngs_cvrd_cbn2}}} and {{{perc_opngs_cvrd_cbn3}}} of foundings, respectively.
#
...
*** Variable selection
:PROPERTIES:
:ID:       569b33ac-c609-4fd0-be65-98b9617a891a
:CUSTOM_ID: vrbl_slctn
:END:
...
#
This procedure results in 36 possible variable combinations (4 HNWI variables \times 3 income inequality variables \times 3 wealth inequality variables). 
...
#
# To obtain comparability between the models when using different time-lags, I limit the country-years to those that have data for all five preceding years.
...
# (the interaction between tax deductibility of charitable donations and top marginal income tax rate and the squared terms of other variables are calculated after rescaling).
#
# {{{mdlcnt_ttl}}} models are run, of which {{{cvrgd1}}} ({{{cvrgnc_rate}}}%) converge successfully.
In total {{{mdlcnt_ttl}}} models are run, all of which converge successfully. 
#
Given 36 variable combinations which are optimized four times with different starting values each per three datasets this results in an average of {{{nbr_runs_p_cbn_spec}}} regression models per variable combination.
#
...
Summary statistics for all variables are provided in unscaled form in table {{{rtbl_descs}}}, providing two relevant insights. 
#
...
#
For example, while in "DS all IVs" on average each year one private museum is founded per {{{nbr_pop_p_1pm_cbn1}}} million inhabitants (1/{{{opng_rate_cbn1}}}), this rate is substantially lower in "DS --CuSP" and "DS --CuSp/TMITR" with one private museum founding per {{{nbr_pop_p_1pm_cbn2}}} and {{{nbr_pop_p_1pm_cbn3}}} million inhabitants, respectively.
# [fn::These rates refer to country-year averages which weigh all units evenly regardless of population size. The dataset-year private museum founding rates are lower at on average one private museum founding per {{{opng_p1m_glbl_yearly_cbn1}}}, {{{opng_p1m_glbl_yearly_cbn2}}}, {{{opng_p1m_glbl_yearly_cbn3}}} million inhabitants in "DS all IVs", "DS --CuSP", and "DS -CuSp/TMITR", respectively, indicating that private museum founding takes place disproportionally in country-years with below average population size.]
#
...
#
Only {{{nbr_nzero_cbn1}}} country-years see at least one private museum founding in "DS all IVs" ({{{prop_nzero_cbn1}}}%), while the other datasets see even lower proportions ({{{prop_nzero_cbn2}}}% for "DS --CuSP" and {{{prop_nzero_cbn3}}}% for "DS --CuSP/TMTIR", respectively).
...
#+INCLUDE: "./tables/tbl:regrslts_wcptblF_v27.tex" export latex



Table {{{rtbl_regrslts_wcptblF}}} displays the regression results. 
#
As the estimation procedure involves the calculation of multiple models from different variable sets, table {{{rtbl_regrslts_wcptblF}}} displays for each variable and dataset the coefficient of the best-fitting model[fn::Coefficient variation due to different variable choice (e.g. the difference between the coefficients of cultural spending between a model with income inequality measured once with the Gini coefficient and once with 10% concentration) generally does not result in qualitatively different coefficients. This and other robustness checks are reported the supplemtary materials.]; the log likelihoods similarly correspond to the best-fitting model. 
# FIXME: for additional model discussion such as coefficient variation due to variable choice, diagnostics, marginal model improvement and mediation see appendix X
...
# *rewrite with focus on cbn1*
# The results for tax incentives largely support H1: 
# #
# Tax deductibility of donations is found to be positive and marginally significantly (p = {{{Ind_tax_incentives_cbn3_pvlu}}}) related to private museum founding in "DS --CuSp/TMITR", indicating a private museum founding rate $e$^{{{{Ind_tax_incentives_cbn2_coef}}}} = {{{Ind_tax_incentives_cbn3_coef_exp}}} times higher in countries where charitable deductions are tax deductible than in countries where these donations are not tax deductible.
# #
...
# #
# In "DS all IVs", the interaction is still positive, but no longer significant, possibly because "DS all IVs" is primarily composed of countries in which charitable donations are tax deductible (table {{{rtbl_descs}}}) and hence shows little variation on this variable. 
# #
...
#
Figure {{{rplt_pred_taxinc}}} visualizes how the impact of top marginal income tax rates depends on the presence of tax deductibility of donations.
#
...
#
A positive role of tax incentives is further supported by the positive (but insignificant; p = {{{Ind_tax_incentives_cbn3_pvlu}}}) coefficient of tax deductibility of donations in DS --CuSp/TMITR, indicating that countries with tax deductibility of donations see $e$^{{{{Ind_tax_incentives_cbn3_coef}}}} = {{{Ind_tax_incentives_cbn3_coef_exp}}} times more private museum founding than countries without. 



However, it is worth noting that in DS all IVs and DS --CuSp at lower top marginal income tax rates countries without tax deductibility of charitable donations see more private museum foundings (figure {{{rplt_pred_taxinc}}}).
#
...
#
While at lower values the association is positive, the effect decreases at higher levels and eventually turns negative (figure {{{rplt_pred_smorc}}}).
#
In particular, up to a value of {{{smorc_lin_flipped}}}/(2*{{{smorc_sqrd}}}) = {{{smorc_top_point_std}}} (corresponding to cultural spending of {{{smorc_top_point}}} USD per capita, approximately the 2020 government cultural spending of Latvia ({{{smorc_2020_LVA}}} USD) or Australia ({{{smorc_2020_AUS}}} USD) an increase in government spending corresponds to a higher predicted likelihood of private museum founding.


#
This range of positive association between government spending and private museum founding consists of {{{smorctop_below_CYs_cnt}}} ({{{smorctop_below_CYs_prop}}}%) of the country-years belonging to "DS all IVs"; it also contains {{{smorctop_below_nbr_opened_prop}}}% of covered private museum foundings.
#
...
#
A 1 SD increase of the income share of the top 10% ({{{sptinc_1SD_cbn1}}}% in 2020 in "DS all IVs", which corresponds approximately to the difference between Sweden ({{{sptinc_iso3c1}}}%) and Canada ({{{sptinc_iso3c2}}}%), or the within-country change in Bulgaria from {{{sptinc_lngtd_year1}}} ({{{sptinc_lngtd_vlu_year1}}}%) to {{{sptinc_lngtd_year2}}} ({{{sptinc_lngtd_vlu_year2}}}%)) is associated with a decrease in private museum founding rate by a factor of $e$^{{{{sptinc_cbn1}}}} = {{{sptinc_cbn1_exp}}}. 
# 
Conversely, a 1 SD increase of the wealth share of the top 10% ({{{shweal_1SD_cbn1}}}%; a difference in size similar to that between Denmark ({{{shweal_iso3c1}}}%) and the Czech Republic ({{{shweal_iso3c2}}}%) in 2020 in "DS all IVs", or to the longitudinal change in the United States from {{{shweal_lngtd_year1}}} ({{{shweal_lngtd_vlu_year1}}}%) to {{{shweal_lngtd_year2}}} ({{{shweal_lngtd_vlu_year2}}}%)) is associated with an increase in private museum founding rate by a factor of $e$^{{{{shweal_cbn1}}}} = {{{shweal_cbn1_exp}}}.
#
There is thus substantial evidence for H3 in regards of wealth inequality, but also strong empirical evidence against a positive association between income inequality and private museum founding.[fn:: While the inequality coefficients are large, it is necessary to consider that wealth and income inequality developments are often related:
#
...
#
[[textcite:&Mijs_2019_paradox]] argues that income inequality in particular increases beliefs in a meritocratic society, which in turn could result in lower demands to justify inequalities towards the economic elites.
#
...
# #
# # Either one or both of these patterns possibly lead to the gini coefficients in "DS all IVs" to become insignificant;
# Investigating the mechanisms behind these patterns constitutes fruitful approaches for future research. 
...
#
In these two datasets, the coefficient of the HNWI population with a wealth larger than 5 million USD in particular is substantially negative ({{{hnwi_nbr_5M_cbn2_coef}}} and {{{hnwi_nbr_5M_cbn3_coef}}}) and significant (p = {{{hnwi_nbr_5M_cbn2_pvlu}}} and p = {{{hnwi_nbr_5M_cbn3_pvlu}}} for DS -CuSp and DS -CuSP/TMITR, respectively).
#
...
A possible explanation for this might be found when investigating the origin of the original argument[fn::Another more technical explanation based on the model specification, in particular the overcontrolling of elite wealth effects with other variables is discussed in the supplementary materials as I consider it less likely.].
#
The main proponents of the link between HNWI and private museum founding, [[textcite:&Gnyp_2015_collectors]], [[textcite:&Zorloni_Resch_2016_opportunities]] and to a lesser extent [[textcite:&Walker_2019_collector]], based their argument primarily on research with collectors and existing museums, rather than studying wealth development more broadly.
#
...
# 
# Some evidence for such speculation is that the HNWI coefficients are positive in "DS all IVs", which 
...
#
Results are shown in figure {{{rplt_cntrfctl}}} (the longitudinal changes of the predictor variables are discussed in the supplementary materials).
#
...
#
Furthermore, for {{{hief_nbr_cry_imputed}}} countries for which the HIEF dataset is not available, I impute it based on the Ethnic Fractionalization in the year 2000 [[parencite:&Alesina_etal_2003_fractionalization]], as both variables are strongly correlated (r={{{cor_hief_eth2k}}}).
