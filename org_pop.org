...
The coefficient of GDP per capita is positive and significant in all datasets, and is furthermore substantially larger in "DS -CuSp" and "DS -CuSP/TMITR". 
#
Given that in these datasets GDP per capita has a smaller standard deviation (table \ref{tbl:descs}), and hence lower variations of GDP are associated with larger changes, there is substantial support for a so far not theorized role of country features such as productivity or education.
#
It thus constitutes a worthwile approach to consider in more detail how private museum founding is related to the institutional environment. 
